document.addEventListener("DOMContentLoaded", function () {
    ...
    // ALAMAT LAMA DARI DATABASE

    const alamatLamaDatabase = alamat.value.trim();

	// AMBIL DETAIL ALAMAT DARI ALAMAT LAMA
    function ambilAlamatLama() {
        ...
        let detail =text.replace(/\bRT\s*\.?\s*[0-9]{1,4}\b/gi,'')
                .replace(/\bRW\s*\.?\s*[0-9]{1,4}\b/gi,'').trim();
    ...
    function loadProvinsi() {
        ...
        provinsi.innerHTML = '<option value="">Memuat Provinsi...</option>';
        ...
        .then(function(data) {
            ...
            if (oldProvinsi) {
                provinsi.value = oldProvinsi;
                loadKabupaten(oldProvinsi);
            }
        })

        .catch(function(error) {
            console.error("ERROR PROVINSI:", error);
			provinsi.innerHTML ='<option value="">Gagal memuat provinsi</option>';
        });
    }
    ...
    function loadKabupaten(idProvinsi) {
        ...
        kabupaten.innerHTML = '<option value="">Memuat Kabupaten/Kota...</option>';
        ...
        fetch(API +"/regencies/" + idProvinsi +".json")
        .then(function(response) {
            if (!response.ok) {
                throw new Error("HTTP Error " + response.status);
            }
        ...
        .then(function(data) {
            ...
            if (oldKabupaten) {
                kabupaten.value = oldKabupaten;
                loadKecamatan(oldKabupaten);
            }
        })

        .catch(function(error) {
            console.error("ERROR KABUPATEN:", error);
            kabupaten.innerHTML ='<option value="">Gagal memuat kabupaten</option>';
        });
    }
    ...
    function loadKecamatan(idKabupaten) {
        ...
        kecamatan.innerHTML = '<option value="">Memuat Kecamatan...</option>';
        desa.disabled = true;
        desa.innerHTML = '<option value="">-- Pilih Desa/Kelurahan --</option>';

        fetch(API + "/districts/" + idKabupaten + ".json")
        .then(function(response) {
        ...
        .catch(function(error) {
            ...
            kecamatan.innerHTML = '<option value="">Gagal memuat kecamatan</option>';
        });
    }
    ...
    function loadDesa(idKecamatan) {
        ...
        desa.innerHTML ='<option value="">Memuat Desa/Kelurahan...</option>';
        fetch(API +"/villages/" + idKecamatan +".json")
        .then(function(response) {
        ...
        .catch(function(error) {
            ...
            console.error("ERROR DESA:", error);
            desa.innerHTML ='<option value="">Gagal memuat desa</option>';
        });
    ...
    function updateAlamat() {
        const provinsiText = provinsi.options[
                provinsi.selectedIndex]?.text || "";

        const kabupatenText = kabupaten.options[
                kabupaten.selectedIndex]?.text || "";

        const kecamatanText = kecamatan.options[
                kecamatan.selectedIndex]?.text || "";

        const desaText = desa.options[
                desa.selectedIndex]?.text || "";

        const jalan = alamatManual.value.trim();
		...
		// DESA
        if (desa.value) {
            bagian.push(desaText );
        }

		// KECAMATAN
        if (kecamatan.value) {
            bagian.push(kecamatanText);
        }

		// KABUPATEN
        if (kabupaten.value) {
            bagian.push(kabupatenText);
        }

		// PROVINSI
        if (provinsi.value) {

            bagian.push(provinsiText);
        }

		// MASUKKAN KE ALAMAT CUCU
        alamat.value = bagian.join(', ');
    }
    ...
    // EVENT DETAIL ALAMAT
    alamatManual.addEventListener("keyup", updateAlamat);
    inputRT.addEventListener("keyup", updateAlamat);
    inputRW.addEventListener("keyup", updateAlamat);

	// AMBIL DATA ALAMAT LAMA
    ambilAlamatLama();
	...
	// LOADING TOMBOL
	document.addEventListener("DOMContentLoaded", function() {
	const form = document.getElementById("formEditCucu");
...
document.addEventListener("DOMContentLoaded", function () {
    const mapElement = document.getElementById("mapEditCucu");
    ...
    const map = L.map("mapEditCucu").setView([lat, lng], 17);
